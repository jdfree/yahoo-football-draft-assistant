...
| **Q6** | Position limits | Caps scale with the queue: **`QUEUE_SIZE / 2`** for skill positions and **`QUEUE_SIZE / 4`** for K and DEF, floored, minimum 1. At `QUEUE_SIZE` 10 that is 5 and 2. A cap set just under the queue size does not do the job a cap is for — the queue must keep offering a genuine alternative when a run empties one position. The K/DEF share leaves a pick **and a fallback**, which matters because there is only ever one kicker worth having and one sniped between the rebuild and our clock would leave nothing behind him. Neither cap depends on back-to-back picks; **Q10** handles that at the draft click. (Below `QUEUE_SIZE` 8 the K/DEF share floors to 1 and the fallback is lost.) **Exception:** with two picks or fewer left and none of that position rostered, the K or DEF cap lifts entirely. **V11** stops deferring a required position once it no longer fits in the picks remaining and from then on offers *only* that position, which the cap contradicted — the plan wanted a queue of kickers, the cap allowed two, and prune removed the rest, every cycle. The cap guards against a run leaving the queue useless; that reasoning does not apply when there is nothing else we are allowed to draft. | — |

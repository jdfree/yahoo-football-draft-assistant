...
| **Q6** | Position limits | Caps scale with the queue: **`QUEUE_SIZE / 2`** for skill positions and **`QUEUE_SIZE / 4`** for K and DEF, floored, minimum 1. At QUEUE_SIZE 10 that is 5 and 2. The old cap of `QUEUE_SIZE − 2` sat just under the queue itself, so one position could take 8 of 10 slots and defeat the point of having a cap — live it reached 6 of 8. The K/DEF share still leaves a pick **and a fallback**, which matters because there is only ever one kicker worth having and one sniped between the rebuild and our clock would otherwise leave nothing behind him. Neither cap depends on back-to-back picks: **Q10** prevents a doubled position at the draft click, and collapsing the K/DEF cap on a pair only stripped the fallback out of every turn-slot queue. (Below QUEUE_SIZE 8 the K/DEF share floors to 1 and that fallback is lost.) | — |
| **Q11** | Missing rows | A player whose row cannot be found after searching is *probably* drafted, but it is an inference. A single miss used to mark him drafted **permanently** — the search racing the render, a filter still applied, a name that did not match — and nothing ever revisited the verdict, so an available player left the pool for good. Live: T. McBride was marked drafted at 02:43:13; he was actually taken at **pick 16, two minutes later**, after our pick 15 — so the pair split took the second best tight end while the best was still on the board. A miss now suppresses him for one cycle only (enough to stop refill retrying the same names every tick); the **second consecutive** miss is what marks him drafted, and finding his row clears the count. | — |
...
| **Q10** | Back-to-back split | On the **second** of two consecutive picks, with `PAIR_SPLIT_AT_SECONDS` left on the clock, draft the top queued player at a **different position** from the one just taken. Yahoo drafts the queue top when a clock expires, and that top rarely moves in the seconds between two consecutive picks, so a turn can spend both on the same position. Fires whether or not `AUTOPICK_AT_SECONDS` is enabled, and only with the clock nearly gone, so your own pick always takes precedence. "Second of a pair" is detected from the **roster**: it is snapshotted when the turn begins, and a position that has grown while the turn is still running is the one just taken. Reading the picks feed instead failed outright — across back-to-back picks Yahoo keeps "your turn" continuous and the header lags, so at the second pick it still reported the first pick's number, and the tick does not sync the picks feed during our own turn anyway. Live, both picks went RB and the rule never fired. A second, worse failure followed: the rule armed correctly (`already took K this turn — will split at 1s`) and then never fired, and **two kickers went with consecutive picks**. The cause was `secondsLeft()`, not the rule. The clock reads `0:50` for most of a turn but drops the colon for the final ten seconds, and the scan ranked candidates "colon first, then topmost" — so it tracked the clock all turn and silently switched to a bare number sitting higher in the draft header exactly when it mattered. `left` floored at that number, never reached 1, and the split never ran. The clock is now identified once and re-read: a colon reading is unambiguous, and failing that the countdown is the element whose value **decreased** since the last scan. A split that is armed also traces the clock once per second, because `draftDifferentPosition` logs on every exit path — so silence meant it was never called, but nothing showed why. Note this rule is now the ONLY guard against a doubled position on a pair, since Q6's back-to-back cap was removed. | `PAIR_SPLIT_AT_SECONDS: 1` |

...
(function () {
  ...
  // Strategy boundary
  //
  // Everything above this line talks to Yahoo. Everything the strategy does is
  // behind these four calls, and it is handed a plain snapshot rather than
  // reaching into `state`, the DOM, or the config directly — so the algorithm
  // can be replaced without touching a line of draft-room code, and the
  // draft-room code can change without the algorithm noticing.
  //
  // Provide `window.YS_STRATEGY` before this script loads to use your own.
  // See strategy.js for the interface and the shape of `ctx`.
  // ===========================================================================

  const STRATEGY = (() => {
    const s = window.YS_STRATEGY || window.YS_DEFAULT_STRATEGY;
    if (!s) {
      throw new Error('no strategy loaded — strategy.js must run before queue-manager.user.js');
    }
    // Check the shape up front. A strategy missing a method would otherwise fail
    // in the middle of a draft, at whichever pick first needed it.
    for (const m of ['baselines', 'project', 'rank', 'plan']) {
      if (typeof s[m] !== 'function') throw new Error(`strategy "${s.name || '?'}" has no ${m}()`);
    }
    return s;
  })();

  /**
   * The snapshot handed to the strategy. Built fresh per call, because the board
   * moves under us; `queued` is a getter because valuing the live queue means
   * running the ranking, which most calls do not need.
   */
  function algoCtx() {
    return {
      pool: state.pool,
      ...
      reserveBaseline: state.reserveBaseline,
      ...
      pickNo: draftPosition().overall,
      round: roundNow(),
      backToBack: backToBack(),
      teamContext: window.YS_TEAM_CONTEXT,
      get queued() { return queueView(); },
    };
  }

  const rankAvailable = (extra) => STRATEGY.rank(algoCtx(), CFG, extra);
  const planQueue = (n, seed = null) => STRATEGY.plan(algoCtx(), CFG, n, seed);
  const projectAvailability = (from, to) => STRATEGY.project(algoCtx(), CFG, from, to);
  ...
  function computeBaseline() {
    ...
    // The bars themselves are the strategy's business (S4); this function only
    // owns the snapshot and the once-only guard.
    const { starter: baseline, reserve: reserveBaseline } =
      STRATEGY.baselines(state.baselinePool, CFG);
